...
app.post("/api/cows/:cow_id/calciulate_tests", authorizeUser, (req, res) => {
    ...
    async.forEachOf(req.body.calciulate_tests, function(calciulate_test, i, callback){
        ...
        getConnection().query("SELECT * FROM cow WHERE cow_id = ? AND user_id = ?", [encrypt(req.params.cow_id), encrypt(req.header("user_id"))], (err, rows, fields) => {
            ...
            calciulate_testValues.push(encrypt(calciulate_test.id))
            calciulate_testValues.push(encrypt(calciulate_test.calciulate_test_id))
            ...
            calciulate_testValues.push(encrypt(req.params.cow_id))
            calciulate_testValues.push(encrypt(req.header("user_id")))
